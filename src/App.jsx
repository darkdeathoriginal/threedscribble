  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        let shared;
        if (!cancelled && items.length > 1) {
          setProcessingLabel('Matching shared strands between images…');
        ...
        if (!cancelled) {
          setScribble({ items, shared, strokes: shared?.strokes ?? items[0].strokes });
          setProcessing(false);
    ...
    <main className="app-shell" onDragOver={event => { event.preventDefault(); if (!videoOpen && event.dataTransfer.types.includes('Files')) setDragging(true); }} onDragLeave={event => { if (!event.currentTarget.contains(event.relatedTarget)) setDragging(false); }} onDrop={event => { event.preventDefault(); setDragging(false); if (!videoOpen) addFiles(event.dataTransfer.files); }}>
      ...
      <div className="workspace">
        <aside className={`control-panel ${panelOpen ? 'is-open' : ''}`}>
          ...
          <div className="panel-footer"><Box size={17} /><p>Shared 3D strands reshape between images. The tour moves into the details.</p></div>
        </aside>
        <section className="gallery">
          <div className="collection-controls"><div role="group" aria-label="Artwork mode"><button aria-pressed={layout === 'together'} className={layout === 'together' ? 'active' : ''} onClick={() => { setLayout('together'); selectView('front'); }}>3D reveal ({images.length > 1 ? images.length - 1 : 1})</button><button aria-pressed={layout === 'single'} className={layout === 'single' ? 'active' : ''} onClick={() => { setLayout('single'); selectView('front'); }}>Single image</button></div><span>{visibleImages.length > 1 ? 'Shared strands · Rotate to reshape · Tour for close-ups' : layout === 'single' ? 'Inspect the selected image' : 'Add images to weave into the same strands'}</span></div>
